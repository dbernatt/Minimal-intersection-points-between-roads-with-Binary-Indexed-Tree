...
void abrazol(elek*& el, int elsz, int n, int m){
	...
	CircleShape* nyugatiVaros = new CircleShape[n+1];
	CircleShape* keletiVaros = new CircleShape[m+1];
	draw_city* nyugatiVarosCoord = new draw_city[n+1];
	draw_city* keletiVarosCoord = new draw_city[m+1];
...
{	
	...
	do{
		jo = false;
		cout << " - [  ] Add meg az allomany nevet : "; cin >> fnev;
			...
			case '3':{
				...
				while(ok2){
					...
					switch(feladat){
						...
						case '5':{
							for (int i = 0; i < betest; ++i) urit(filenames[i]);
						} break;
